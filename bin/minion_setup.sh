...
[ -f /vagrant/etc/minion.conf ] || die "File: /vagrant/etc/minion.conf does not exist"

sudo sed 's/#master: salt/master: '${1}'/g' /vagrant/etc/minion.conf | sudo tee /etc/salt/minion > /dev/null
